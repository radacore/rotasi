...
class PdfBookViewer extends StatefulWidget {
  ...
  /// Path file PDF lokal (offline). Bila diisi, PDF dibaca langsung dari
  /// storage perangkat tanpa jaringan (tidak lewat `http`).
...
class _PdfBookViewerState extends State<PdfBookViewer>
  ...
  Future<void> _load() async {
    ...
    if (filePath != null) {
      await pdfLoader.loadPdfFile(filePath);
    } else {
  ...
  void _onPageChanged() {
    if (widget.onPageChanged != null) {
      if (currentPage != appState.currentPageComplete + 1 ||
          totalPages != appState.currentTotalPages) {
        currentPage = appState.currentPageComplete + 1;
        totalPages = appState.currentTotalPages;
                      ...
                      : Expanded(
                          child: LayoutBuilder(
                            builder: (context, constraints) {
                              ...
                              /// Scale down if single page is too wide for screen
