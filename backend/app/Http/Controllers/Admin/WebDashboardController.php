...
class WebDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $symptomAlerts = SymptomCheck::where(function ($q) {
            $q->where('headache', 1)
                ->orWhere('blurred_vision', 1)
                ->orWhere('epigastric_pain', 1)
                ->orWhere('shortness_of_breath', 1);
        })->count();
        ...
        $alertPatientIds = collect($bpPatientIds)->merge($symptomPatientIds)->unique()->values();

        $attentionPatients = Patient::query()
            ->where(function ($q) use ($alertPatientIds) {
        ...
        return inertia('Dashboard', [
            'stats' => [
                ...
                'bp_alerts' => $bpAlerts,
                'bp_patients' => $bpPatientIds->count(),
                'symptom_alerts' => $symptomAlerts,
                'symptom_patients' => $symptomPatientIds->count(),
                'sync_7d' => $sync7d,
                // compat for old frontend/tests if any
                'active_booklet_version' => 0,
                'booklet_releases' => 0,
                'apk_releases' => 0,
